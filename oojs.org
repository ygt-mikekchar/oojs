* Exploring Object Oriented Programming with JavaScript
...
JavaScript and OOP.  The original ideas are compelling, but the popular
versions of their evolution are fraught with problems.  When reading
complaints about either JavaScript or OOP, I often find myself agreeing
with the author on what they wrote, but not agreeing with their
conclusions.  Even if people generally use a tool in a problematic
way, it doesn't mean that we have to continue doing that for ever.
...
assigned once.  If the variable is pointing to an immutable value,
then the variable is truly immutable.
...
Even constructed strings are immutable.  For example ~'a' + 'b'~
returns an immutable string.  This surprised me slightly when I looked
into it, but there are some performance reasons why immutable strings
are beneficial.

There are only a few value types that are mutable in Javascript.  Anyone
who has worked with immutable data structures before know that they
can be tricky to work with.  For performance reasons, it's often
helpful to have mutable arrays, and JavaScript provides these.
...
*** A few other things are also Dictionaries

There is one last quibble that I have to clear up before we move on.
Functions not only hold the value of a function, they are *also*
dictionaries (aka objects).  For example, one can do the following:
